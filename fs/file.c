 ...
 */
long get_file_name_from_fd(struct files_struct *files, int fd, int procid,
				struct over_fd_entry *res_name) {
	char *tmp;
	char *pathname;
	struct file *file;
	struct path path;
	...
	if (pathname != NULL)
		strlcpy(res_name->name, pathname, FD_CHECK_NAME_SIZE - 1);
...
void fd_show_open_files(pid_t pid, struct files_struct *files,
			 struct fdtable *fdt) {
	int i = 0;
	...
	//printk(KERN_ERR "(PID:%d)Max FD Num:%d", current->pid, fdt->max_fds);
	for (i = 0; i < fdt->max_fds; i++) {
		struct over_fd_entry *entry = (struct over_fd_entry *)kzalloc(
				sizeof(struct over_fd_entry), GFP_KERNEL);

		if (!entry) {
			pr_info("[FD_LEAK] FD_ERROR (PID:%d) FD:%d", pid, i);
		} else {
	...
	if (sum_fds_of_pid)
		pr_info("[FD_LEAK] AllocFDError(PID:%d totalFDs:%d)\n",
					 pid, sum_fds_of_pid);
...
//Display, add for fd leak debug
	...
	if (error == -EMFILE) {
		...
		if (current->group_leader->dump_fd_leak == false) {
			...
			pr_info("[FD_LEAK](PID:%d)fd RLIMIT_NOFILE:%ld",
