...
just_print:
			seq_printf(m, "tsk: %s, %d, adj %d, size %d\n",
				tsk->comm, tsk->pid, tsk->signal->oom_score_adj, tasksize);
		}
	}
	rcu_read_unlock();
	return 0;
...
{
	proc_create("adj_chain_stat", S_IFREG | 0400, NULL, &adj_chain_proc_fops);
	proc_create("adj_chain_verify", S_IFREG| 0400, NULL, &adj_chain_verify_proc_fops);
	return 0;
}
