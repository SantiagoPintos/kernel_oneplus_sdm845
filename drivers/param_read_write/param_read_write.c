...
static int param_set_test(const char *val, const struct kernel_param *kp)
{
	int ret;
...
static int param_set_angela(const char *val, const struct kernel_param *kp)
{
...
static int param_get_angela(char *val, const struct kernel_param *kp)
{
...
static int param_set_data_stage(const char *val, const struct kernel_param *kp)
{
	int data_in_ufs = 0;
...
static int param_get_data_stage(char *val, const struct kernel_param *kp)
{
...
static int cal_rebootcount = -1;
static int param_set_cal_rebootcount(const char *val, const struct kernel_param *kp)
{
	uint32 sid_index = PARAM_SID_SALEINFO;
	uint32 offset = offsetof(param_saleinfo_t, cal_rebootcount);

	sscanf(val, "%d", &cal_rebootcount);
	set_param_by_index_and_offset(sid_index, offset, &cal_rebootcount, sizeof(cal_rebootcount));

	return 0;
}
module_param_call(cal_rebootcount, param_set_cal_rebootcount, NULL, &cal_rebootcount, 0644);

/* liochen@BSP, 2017/07/17, Add param unlock_count */
static int unlock_count = 0;
static int param_get_unlock_count(char *val, const struct kernel_param *kp)
{
        int cnt = 0;
        uint32 sid_index= PARAM_SID_PHONE_HISTORY;
        uint32 offset = offsetof(param_phonehistory_t, unlock_count);

        get_param_by_index_and_offset(sid_index,offset, &unlock_count, sizeof(unlock_count));
        cnt = sprintf(val, "%d", unlock_count);
        pr_err("param_get_unlock_count(): update_count=%d\n", unlock_count);
...
static int param_get_cust_flag(char *val, const struct kernel_param *kp)
{
        int cnt = 0;
        uint32 sid_index= PARAM_SID_SALEINFO;
        uint32 offset = offsetof(param_saleinfo_t, cust_flag);

        get_param_by_index_and_offset(sid_index,offset, &cust_flag, sizeof(cust_flag));
        cnt = sprintf(val, "%d", cust_flag);
        pr_err("param_get_cust_flag(): cust_flag=%d\n", cust_flag);
...
static int backcover_color = 0;
static int param_get_backcover_color_flag(char *val, const struct kernel_param *kp)
{
        int cnt = 0;
        uint32 sid_index= PARAM_SID_SALEINFO;
...
static int param_set_backcover_color_flag(const char *val, const struct kernel_param *kp)
{
...
module_param_call(backcover_color, param_set_backcover_color_flag, param_get_backcover_color_flag, &backcover_color, 0664);

/* liochen@BSP, 2017/06/30, Add normal_reboot_count and abnormal_reboot_count */
static int normal_reboot_count = 0;
static int param_set_normal_reboot_count(const char *val, const struct kernel_param *kp)
{
        uint32 sid_index= PARAM_SID_PHONE_HISTORY;
...
static int param_get_normal_reboot_count(char *val, const struct kernel_param *kp)
{
        int cnt = 0;
        uint32 sid_index= PARAM_SID_PHONE_HISTORY;
        uint32 offset = offsetof(param_phonehistory_t, normal_reboot_count);
...
static int param_set_abnormal_reboot_count(const char *val, const struct kernel_param *kp)
{
        uint32 sid_index= PARAM_SID_PHONE_HISTORY;
...
static int param_get_abnormal_reboot_count(char *val, const struct kernel_param *kp)
{
        int cnt = 0;
        uint32 sid_index= PARAM_SID_PHONE_HISTORY;
        uint32 offset = offsetof(param_phonehistory_t, abnormal_reboot_count);
...
static int param_set_update_count(const char *val, const struct kernel_param *kp)
{
	uint32 sid_index= PARAM_SID_PHONE_HISTORY;
...
static int param_get_update_count(char *val, const struct kernel_param *kp)
{
	int cnt = 0;
	uint32 sid_index= PARAM_SID_PHONE_HISTORY;
	uint32 offset = offsetof(param_phonehistory_t, update_count);
...
static int param_get_fastboot_count(char *val, const struct kernel_param *kp)
{
        int cnt = 0;
        uint32 sid_index= PARAM_SID_PHONE_HISTORY;
...
int add_restart_08_count(void)
{
    ...
    ret = get_param_by_index_and_offset(sid_index, offset, &restart_08_count, sizeof(restart_08_count));

    restart_08_count = restart_08_count + 1;

    ret = set_param_by_index_and_offset(sid_index, offset, &restart_08_count, sizeof(restart_08_count));
...
int add_restart_other_count(void)
{
    ...
    ret = get_param_by_index_and_offset(sid_index, offset, &restart_other_count, sizeof(restart_other_count));

    restart_other_count = restart_other_count + 1;

    ret = set_param_by_index_and_offset(sid_index, offset, &restart_other_count, sizeof(restart_other_count));
