...
unsigned long memplus_scan(void)
{
	...
	spin_lock(&swap_lock);
	plist_for_each_entry_safe(sis, next, &swap_avail_head, avail_list) {
		spaces = &swapper_spaces[sis->type];
		if (!spaces)
			continue;

		index = 0;
		pagevec_init(&pvec, 0);
		while (pagevec_lookup_entries(&pvec, spaces, index, (pgoff_t)PAGEVEC_SIZE, indices)) {
